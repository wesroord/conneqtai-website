const P=[
  ...
  {i:"🎯",n:"Support",t:"Beste waarde",p:"2.497",m:"197",d:"Volledige AI-klantenservice die e-mails en chat automatisch afhandelt — zodat jouw team zich richt op wat telt.",f:["Alles uit Chatbot","AI e-mail en chat support","Auto-escalatie naar je team","HubSpot CRM-integratie","Maandelijkse rapportage"],h:true},
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,240px),1fr))",gap:"16px"}}>
          {P.map(p=>(
            <div key={p.n} style={{padding:"28px 22px",borderRadius:"16px",border:p.h?"1px solid rgba(255,0,170,.5)":"1px solid rgba(255,255,255,.1)",background:p.h?"linear-gradient(135deg,rgba(255,0,170,.1),rgba(119,0,255,.07))":"rgba(255,255,255,.03)",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column"}}>
              ...
              <a href={"mailto:hello@conneqtai.com?subject=Interesse in het "+p.n+" pakket&body=Hallo ConneqtAI,%0D%0A%0D%0AIk heb interesse in het "+p.n+" pakket.%0D%0A%0D%0AKun je contact met mij opnemen?%0D%0A%0D%0AMet vriendelijke groet,"} style={{display:"block",textAlign:"center",padding:"12px 16px",borderRadius:"8px",background:p.h?"linear-gradient(135deg,#ff00aa,#7700ff)":"rgba(255,255,255,.08)",border:p.h?"none":"1px solid rgba(255,255,255,.15)",color:"#fff",textDecoration:"none",fontFamily:"'Inter',sans-serif",fontSize:"13px",fontWeight:600}}>Neem contact op →</a>
            </div>
          ))}
        </div>
