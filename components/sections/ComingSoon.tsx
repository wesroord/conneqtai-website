...
export default function ComingSoon({ lang = "en" }: { lang?: string }) {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({length:80}, () => ({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-.5)*.15,vy:(Math.random()-.5)*.15,r:Math.random()*1.4+.3,a:Math.random()*.15+.04,ph:Math.random()*Math.PI*2,ps:.002+Math.random()*.004,col:cols[Math.floor(Math.random()*cols.length)]}));
    let raf = 0;
    function frame() {
      ...
      for(let i=0;i<pts.length;i++) for(let j=i+1;j<pts.length;j++) { const dx=pts[i].x-pts[j].x,dy=pts[i].y-pts[j].y,d=Math.sqrt(dx*dx+dy*dy); if(d<90){cx.beginPath();cx.moveTo(pts[i].x,pts[i].y);cx.lineTo(pts[j].x,pts[j].y);cx.strokeStyle="rgba("+pts[i].col+","+String(.05*(1-d/90))+")";cx.lineWidth=.4;cx.stroke();} }
    ...
    <div style={{position:"fixed",inset:0,background:"#000005",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:2,textAlign:"center",padding:"24px",maxWidth:"600px",width:"100%"}}>
        ...
        <div style={{display:"inline-flex",alignItems:"center",gap:"8px",padding:"6px 16px",borderRadius:"100px",border:"1px solid rgba(255,0,170,.3)",background:"rgba(255,0,170,.06)",marginBottom:"28px"}}>
          ...
          <span style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",letterSpacing:".12em",textTransform:"uppercase",color:"rgba(255,0,170,.8)",fontWeight:600}}>{nl ? "Binnenkort beschikbaar" : "Coming soon"}</span>
        </div>
        ...
        {/* Sub */}
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"clamp(15px,2vw,18px)",color:"rgba(255,255,255,.6)",lineHeight:1.75,marginBottom:"40px",maxWidth:"460px",margin:"0 auto 40px"}}>
          {nl
        ...
        {!sent ? (
          <div style={{display:"flex",gap:"10px",justifyContent:"center",flexWrap:"wrap",marginBottom:"40px"}}>
            ...
            />
              ...
              style={{padding:"14px 24px",borderRadius:"10px",background:"linear-gradient(135deg,#ff00aa,#7700ff)",color:"#fff",textDecoration:"none",fontSize:"14px",fontWeight:600,fontFamily:"'Inter',sans-serif",display:"inline-block",position:"relative"}}
            >
              <div style={{position:"absolute",inset:"-2px",borderRadius:"12px",background:"linear-gradient(135deg,#ff00aa,#7700ff,#00aaff)",zIndex:-1,filter:"blur(8px)",opacity:.5}}/>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"20px",flexWrap:"wrap"}}>
          ...
          <a href="https://www.linkedin.com" target="_blank" rel="noopener noreferrer" style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",color:"rgba(255,255,255,.4)",textDecoration:"none"}}>LinkedIn</a>
        </div>
      </div>
    </div>
  );
