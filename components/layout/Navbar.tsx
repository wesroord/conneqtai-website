...
export default function Navbar() {
  ...
  const links = ["Solutions","Process","Results","Demo"];
  return (
    <>
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 22px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:scrolled?"rgba(3,3,10,0.97)":"rgba(3,3,10,0.85)",backdropFilter:"blur(30px)",transition:"background 0.3s"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <button onClick={()=>setMenuOpen(!menuOpen)} className="hamburger" style={{display:"none",background:"none",border:"none",cursor:"pointer",color:"#f0f0f8",fontSize:"1.3rem"}}>{menuOpen?"✕":"☰"}</button>
        </div>
      </nav>
      {menuOpen&&(
        <div style={{position:"fixed",top:"52px",left:0,right:0,zIndex:99,background:"rgba(3,3,10,0.99)",backdropFilter:"blur(24px)",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"2rem",display:"flex",flexDirection:"column",gap:"1.5rem"}}>
          {links.map(l=>(
            <a key={l} href={`#${l.toLowerCase()}`} onClick={()=>setMenuOpen(false)} style={{fontSize:"12px",color:"rgba(240,240,248,0.7)",textDecoration:"none",fontFamily:"'Space Mono',monospace",letterSpacing:".05em"}}>{l}</a>
          ))}
          <a href="mailto:hello@conneqtai.com" onClick={()=>setMenuOpen(false)} style={{fontSize:"13px",fontWeight:600,color:"#fff",textDecoration:"none",padding:"12px 20px",borderRadius:"100px",background:"linear-gradient(135deg,#6c63ff,#a89cff)",textAlign:"center" as const}}>Get in touch →</a>
        </div>
      )}
